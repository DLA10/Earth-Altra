...
quick bite — a model decides which ones are worth biting.
...
Live on paper, running to a fixed review date regardless of interim results — deliberately,
so a bad week can't trigger a panic change. The edge is regime-sensitive: the same dials
...
A related programme that found **no** deployable edge in small-bracket scalping — worth
reading before proposing a new one: [`HARVEST_STUDY.md`](../HARVEST_STUDY.md).
