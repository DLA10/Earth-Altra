...
for what would justify promoting or benching each detector. Those were written down
*before* the results came in, precisely so a hot streak or a bad week can't drive the
decision.
...
Recent addition: every exit now records its **slippage**, meaning the difference between
where the stop was meant to trigger and what the fill actually was. Paper profits mean nothing if the
...
[`SURGER_V2.md`](../SURGER_V2.md) covers the validation across four backtest windows, the
exit study, and the per-detector numbers.
